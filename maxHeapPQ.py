class maxHeapPQ:
    ...
    def upheap(self, heap, i):
        ...
        if p_i < 0:
            return
        # Swap nodes if current is larger than the parent
        if heap[i] > heap[p_i]:
    ...
    def downheap(self, heap, i):
        ...
        # If node has 2 children, then swap w/ larger child node
        if c_i + 1 < len(self.heap) and heap[c_i] < heap[c_i + 1]:
            c_i += 1

        # Swap if child node is less than current node
        if heap[c_i] > heap[i]:
...
def main():
    ...
    while (True):
        if insert:
            print(
                "\nEnter an INTEGER into the Max Heap.\nEnter \"f\" or \"F\" to finish.\n")
            choice = validate()
